function [cost, grad] = ffnnRegCostGrad(dataX, dataY,...
    numNeuronsLayers,...
    theta,...
    transferFunctions,...
    metaParameters)
%% This function computes the cost and the gradient of a feed-forward neural network
...
%
% Outputs
% cost               Contains the cost value of the function to train
%                         the neural network
% grad          Vector containing the gradients of all the
...
%% Forward propagation
[Z, A] = ffnnRegTest(dataX, W, transferFunctions, metaParameters);
dataYHat = A{numLayers}(:,2); % [dataYHat_1; dataYHat_2; ...; dataYHat_N]

%% Cost
% Computing the value of the cost function given the weights of the network
kte = 1/N;
mse = kte*(1/2)*sum( (dataY - dataYHat).^2 ); % MS error
...
grad = zeros(totalNumberWeights,1);
endPoint = 0;
for i = 2:numLayers
    numRows = numNeuronsLayers(i);
    numCols = numNeuronsLayers(i - 1) + 1;
    ...
    startPoint = endPoint + 1;
    endPoint = endPoint + numWeights;
    grad(startPoint:endPoint,1) = Wgradient{i - 1}(:);
end
return
